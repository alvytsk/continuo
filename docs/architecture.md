...
**`library.rs`'s `async` functions still do synchronous filesystem work.** Reading `subscriptions.json`, reading and atomically replacing a cache file, and `fsync`ing both happen on whatever thread polls the future, between `await` points. Today that is harmless: `commands.rs` blocks a `main` that has nothing else to do. It would not be harmless in M5's event loop, which must stay responsive to keystrokes. The terminal player keeps them off the application thread: since M6 the browser subscribes, refreshes and unsubscribes on the browse worker, which runs the same `library.rs` functions the CLI does, and the application thread's only feed reads are the pre-load podcast lookup noted in the table above and the active entry's artwork lookup, resolved only when the entry, its load or the HTTP service changes (`PlayerRuntime::cover_key`). Every mutation holds an exclusive OS lock on `subscriptions.lock`, beside `subscriptions.json`, for its whole read-modify-write, fetch included, so a CLI command and the player cannot save stale snapshots over each other; contention refuses at once with `Another subscription update is in progress`. The browse worker is detached, so a mutation in flight when the player quits finishes on its own; it holds the subscription lock until it commits, and the profile lock is not what protects subscriptions.
