# Tenuto M5: compact Ratatui player
...
The selected browser reference is [compact player](assets/tenuto-compact-reference.html). It contains sample content and simulated playback, not production behavior. Its outer browser page, design-study headings, comparison controls, and window decoration are not part of the terminal application.
...
Add an explicit `tenuto tui` entry point. Keep existing `tenuto play`, feed commands, and `--probe-only` behavior compatible except for the explicitly shared playback locking and signal changes below. A bare `tenuto` retains the required-subcommand usage error and exit code 2; `tenuto --help` remains the successful help entry point.
...
If the lock is held, both a second `tui` and a second `play` refuse to start, exit nonzero, and report `Another Tenuto player is using this state profile` before opening an audio device or entering raw mode. Do not use the `writable = false` path for contention. Failure to resolve/create/lock the state profile is also a startup error for these playback commands. This intentionally replaces the existing no-state-directory unsaved fallback. After successful lock acquisition, the existing read-only/disabled-writer handling for unsupported or unreadable state remains, with a visible unsaved-session warning. Read-only feed listing and probe operations do not acquire this lock; their snapshots are never promoted into a playback writer. These locks coordinate updated Tenuto processes; older binaries that ignore the lock remain outside the guarantee.
