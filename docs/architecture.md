...
M3's `HttpService` bounds the application's own encoded-byte buffer (`Limits::buffer_bytes`) plus one transfer chunk on top of it (`Limits::chunk_bytes`), and that is the total this document claims. reqwest 0.13.5 exposes no read-buffer-size knob for either its HTTP/1.1 or HTTP/2 path (no `http1_max_buf_size` or equivalent), so the HTTP/TLS library's own transport buffering is not bounded by Continuo and is not part of the stated cap — consistent with the design's requirement to document library buffering separately rather than claim a total over process memory.
