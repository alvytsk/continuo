...
fn a_freeze_suspends_the_fetch_tasks_stall_timer() {
    ...
    let waiter = {
        ...
        std::thread::spawn(move || {
            ...
            rt.block_on(interrupt.wait_while_frozen(generation));
        })
    };
    std::thread::sleep(Duration::from_millis(50));
    assert!(
    ...
    runtime().block_on(interrupt.wait_while_frozen(generation));
}

#[test]
fn a_begin_with_no_preceding_retire_still_wakes_a_task_parked_in_wait_while_frozen() {
    // `ensure_source_open` (Task 10) calls `begin()` directly, with no
    // preceding `retire()`. A fetch task parked in `wait_while_frozen` while
    // playback happens to be paused must still reach its own
    // `cancelled(generation)` branch as soon as the generation moves on —
    // not sit here holding the connection open until someone happens to
    // press play.
    let interrupt = SourceInterrupt::new(CAPACITY);
    let generation = interrupt.generation();
    interrupt.freeze();
    ...
    assert!(
        !waiter.is_finished(),
        "wait_while_frozen returned before the generation moved on"
    );
    // No `retire()` here: `begin()` alone must be enough, and the interrupt
    // is still frozen throughout.
    let _next_generation = interrupt.begin();
    match waiter.join() {
        Ok(elapsed) => assert!(elapsed < Duration::from_secs(1), "woke after {elapsed:?}"),
        Err(_) => panic!("the waiter thread panicked"),
    }
