...
| Artwork worker (`continuo-artwork`, `src/artwork/worker.rs`, M5) | One latest-wins request slot: resolves and decodes the active local entry's cover (embedded front cover, then `cover.jpg`, `cover.png`, `folder.jpg`, `folder.png`) within 10 MiB encoded and 16 million decoded pixels, each job inside `run_contained` | Encode for the terminal (the application thread does that, also contained), fetch remote art, or touch `Session` |
...
| **M5** | Ratatui TUI over the existing application interfaces: queue, browser, artwork, spectrum, profile lock, signal contract — shipped |
