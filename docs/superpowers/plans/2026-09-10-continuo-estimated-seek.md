...
- **`docs/m1-known-debt.md`** — correct the M3 entry that says `SeekMode::Accurate` is merely a testing limitation; it was a live defect and this is its fix. Also correct the entry claiming `Coarse` refuses without a Xing header: the spike disproved it both by reading `symphonia-bundle-mp3-0.6.1/src/demuxer.rs:465-471` and by exercising it.

  **M3.1's carried debt, all four items established during execution rather than planned:**

  1. **An MP3 `Coarse` landing has no useful accuracy bound.** Measured error grows from 0.4 s to **235 s on a 600 s file** as the target approaches an estimated ceiling — at a 360 s target it lands at 595 s while self-reporting ~360 s. Bounded only by the file's own length, which is not a guarantee a listener can use. This is why every MP3 landing reports `Estimated`. Requires: no-Xing/Info/VBRI tag **and** genuine VBR. Files with a tag (which includes the podcast from the acceptance report — `Info`, exact frame count, CBR to within 0.03 s over 2 h 16 m) land essentially exactly.
  2. **The tail of an under-estimated file is unreachable, deliberately.** Symphonia's own `max_ts` check refuses a target past its estimated ceiling at `demuxer.rs:268-272`, *before* the mode dispatch at `:292-296` — so the refusal is mode-independent by construction and no seek mode recovers it. Removing our own `clamp_target` converted a silent mislanding into a visible refusal; it did not extend reach. Retained, not a bug to fix here.
  3. **Shapes A and B, rejected by the spike.** Shape A (re-probe at the byte offset) is implementable but needs a codec-specific constructor or a `MediaSource` adapter — `Probe::probe()` cannot probe from an offset, because `MediaSourceStream::new` always initialises `abs_pos` to 0 and the trailing-metadata pre-scan restores to `mss.pos()`. It carries the identical duration-estimate vulnerability plus a bespoke adapter and manual reservoir bookkeeping. Shape B (byte-seek beneath a live reader) is **structurally unimplementable**: `FormatReader`'s only route back to the stream is `into_inner(self: Box<Self>)`, which consumes the reader, and nothing exposes `next_packet_ts`.
  4. **A better duration estimate, not attempted.** `estimate_num_mpeg_frames` samples ~16 frames. Sampling frame headers at ten points across the file would cost ~40 KB against the 983 KB rescan this amendment removes and would collapse the dominant error term in item 1. Symphonia's discarded Xing TOC (parsed into `XingInfoTag.toc`, marked `#[allow(dead_code)]`, never read) would do better still if it were reachable. Out of scope deliberately: this amendment removes a wedge.
...
(`symphonia-bundle-mp3-0.6.1/src/demuxer.rs:749-758`) parses the tag into
